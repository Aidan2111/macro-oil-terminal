"""Visual audit — full-page screenshots + DOM diagnostics across viewports.

Captures four viewports (iphone13 / pixel7 / tablet / desktop) for every
public route and writes the screenshots + a JSON findings dump to
/tmp/visual-audit/.
"""
        ...
        for vp in VIEWPORTS:
            ...
            for route in ROUTES:
                ...
                try:
                    ...
                    page.screenshot(
                        path=f"{OUT}/{vp['name']}_{slug}.png", full_page=True
                    )
                except Exception as exc:
                    diag = {"error": repr(exc)}
                findings.append({"viewport": vp["name"], "route": route, **diag})
                page.close()
            ctx.close()
        browser.close()
