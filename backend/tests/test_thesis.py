...
def patched_compat(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(thesis_service, "_generate_thesis", _fake_generate_thesis)
    monkeypatch.setattr(
        thesis_service, "_build_thesis_context", lambda: types.SimpleNamespace()
    )
    yield
...
def test_thesis_latest_empty(monkeypatch: pytest.MonkeyPatch, tmp_path):
    ...
    monkeypatch.setattr(thesis_service, "_read_recent_theses", lambda n: [])
    client = TestClient(create_app())
...
def test_thesis_latest_returns_newest(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(thesis_service, "_read_recent_theses", lambda n: [record])
    client = TestClient(create_app())
    resp = client.get("/api/thesis/latest")
    assert resp.status_code == 200
    body = resp.json()
...
def test_thesis_history_respects_limit(monkeypatch: pytest.MonkeyPatch):
    ...
    rows = [
        {"timestamp": f"2026-04-{23 - i:02d}T10:00:00Z", "thesis": {"stance": "flat"}}
        for i in range(5)
    ]
    ...
    monkeypatch.setattr(thesis_service, "_read_recent_theses", _fake)
    client = TestClient(create_app())
